...
namespace ImGui
{
  ...
  IMGUI_API void EndFrame(); // ends the Dear ImGui frame. automatically called by Render(). If you don't need to render
                             ...
                             // wasted CPU already! If you don't need to render, better to not create any windows and
  ...
  //    corresponding BeginXXX function returned true. Begin and BeginChild are the only odd ones out. Will be fixed in
  //    a future update_ellipse.]
  ...
  IMGUI_API bool SliderFloatWithSteps(const char* label,
                                      float* v,
                                      float v_min,
                                      float v_max,
                                      float v_step,
  ...
  IMGUI_API bool SliderInt(const char* label,
                           ...
                           const char* format     = "%d",
                           ImGuiSliderFlags flags = 0);
  IMGUI_API bool SliderIntWithSteps(const char* label, int* v, int v_min, int v_max, int v_step);
  ...
  IMGUI_API bool CollapsingHeader(
      const char* label,
      ImGuiTreeNodeFlags flags = 0); // if returning 'true' the header is open. doesn't indent nor push on ID stack.
  ...
  IMGUI_API void SetNextItemAllowOverlap(); // allow next item to be overlapped by a subsequent item. Useful with
                                            ...
                                            // items may need to be added. Note that both Selectable() and TreeNode()
  ...
  IMGUI_API bool IsItemClicked(
      ImGuiMouseButton mouse_button =
          0); // is the last item hovered and mouse clicked on? (**)  == IsMouseClicked(mouse_button) &&
              // IsItemHovered()Important. (**) this is NOT equivalent to the behavior of e.g. Button(). Read comments
  ...
  IMGUI_API void SetNextFrameWantCaptureKeyboard(
      bool want_capture_keyboard); // Override io.WantCaptureKeyboard flag next frame (said flag is left for your
                                   ...
                                   // equivalent to setting "io.WantCaptureKeyboard = want_capture_keyboard"; after the
                                   // next NewFrame() call.
  ...
  IMGUI_API void SetNextFrameWantCaptureMouse(
      bool want_capture_mouse); // Override io.WantCaptureMouse flag next frame (said flag is left for your application
                                // to handle, typical when true it instucts your app to ignore inputs). This is
                                // equivalent to setting "io.WantCaptureMouse = want_capture_mouse;" after the next
                                // NewFrame() call.
...
enum ImGuiWindowFlags_
{
  ...
  ImGuiWindowFlags_UnsavedDocument =
      1 << 18, // Display a dot next to the title. When used in a tab/docking context, tab is selected when clicking the
  ...
  ImGuiWindowFlags_NavFlattened =
      1 << 23, // [BETA] On child window: share focus scope, allow gamepad/keyboard navigation to cross over parent
...
//   BUT it won't update_ellipse its auto-size while clipped.
...
//   - You may also use ImGuiChildFlags_AlwaysAutoResize to force an update_ellipse even when child window is not in
//   view.
...
enum ImGuiChildFlags_
{
  ...
  ImGuiChildFlags_AlwaysUseWindowPadding = 1
      << 1, // Pad with style.WindowPadding even if no border are drawn (no padding by default for non-bordered child
...
enum ImGuiInputTextFlags_
{
  ...
  ImGuiInputTextFlags_EnterReturnsTrue = 1
      << 5, // Return 'true' when Enter is pressed (as opposed to every time the value was modified). Consider looking
  ...
  ImGuiInputTextFlags_CallbackCharFilter = 1
      << 9, // Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or discard, or
            // return 1 in callback to discard.
  ImGuiInputTextFlags_AllowTabInput       = 1 << 10, // Pressing TAB input a '\t' character into the text field
  ImGuiInputTextFlags_CtrlEnterForNewLine = 1
      << 11, // In multi-line mode, unfocus with Enter, add new line with Ctrl+Enter (default is opposite: unfocus with
...
enum ImGuiTreeNodeFlags_
{
  ...
  // ImGuiTreeNodeFlags_NoScrollOnOpen     = 1 << 15,  // FIXME: TODO: Disable automatic scroll on TreePop() if node got
...
enum ImGuiTabBarFlags_
{
  ...
  ImGuiTabBarFlags_NoCloseWithMiddleMouseButton = 1
      << 3, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You
...
enum ImGuiTabItemFlags_
{
  ...
  ImGuiTabItemFlags_NoCloseWithMiddleMouseButton = 1
      << 2, // Disable behavior of closing tabs (that are submitted with p_open != NULL) with middle mouse button. You
            // may handle this behavior manually on user's side with if (IsItemHovered() && IsMouseClicked(2)) *p_open =
            // false.
...
enum ImGuiHoveredFlags_
{
  ...
  ImGuiHoveredFlags_AllowWhenOverlappedByItem = 1
      << 8, // IsItemHovered() only: Return true even if the item uses AllowOverlap mode and is overlapped by another
            // hoverable item.
...
enum ImGuiDragDropFlags_
{
  ...
  ImGuiDragDropFlags_SourceNoDisableHover = 1
      << 1, // By default, when dragging we clear data so that IsItemHovered() will return false, to avoid subsequent
  ...
  ImGuiDragDropFlags_SourceNoHoldToOpenOthers = 1
      << 2, // Disable the behavior that allows to open tree nodes and collapsing header by holding over them while
            // dragging a source item.
  ImGuiDragDropFlags_SourceAllowNullID = 1
      << 3, // Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by
  ...
  ImGuiDragDropFlags_SourceAutoExpirePayload = 1
      << 5, // Automatically expire the payload if the source cease to be submitted (otherwise payloads are persisting
            // while being dragged)
...
enum ImGuiConfigFlags_
{
  ...
  ImGuiConfigFlags_NavEnableSetMousePos = 1
      << 2, // Instruct navigation to move the mouse cursor. May be useful on TV/console systems where moving a virtual
            // mouse is awkward. Will update_ellipse io.MousePos and set io.WantSetMousePos=true. If enabled you MUST
            // honor io.WantSetMousePos requests in your backend, otherwise ImGui will react as if the mouse is jumping
...
enum ImGuiBackendFlags_
{
  ...
  ImGuiBackendFlags_RendererHasVtxOffset = 1
      << 3, // Backend Renderer supports ImDrawCmd::VtxOffset. This enables output of large meshes (64K+ vertices) while
...
//   ("Edit.GoToImplementation") cannot. With Visual Assist installed: ALT+G ("VAssistX.GoToImplementation") can also
//   follow symbols in comments.
// - When changing this enum, you need to update_ellipse the associated internal table GStyleVarInfo[] accordingly. This
// is where we link enum values to members offset/type.
enum ImGuiStyleVar_
{
...
struct ImGuiIO
{
  ...
  // - We expect to update_ellipse the API eventually. In the meanwhile we provide tools to facilitate checking
  // user-code behavior.
  ...
  IMGUI_API void AddMousePosEvent(float x, float y); // Queue a mouse position update_ellipse. Use -FLT_MAX,-FLT_MAX to
                                                     // signify no mouse (e.g. app not focused and not hovered)
  IMGUI_API void AddMouseButtonEvent(int button, bool down); // Queue a mouse button change
  IMGUI_API void AddMouseWheelEvent(
      float wheel_x,
      float wheel_y); // Queue a mouse wheel update_ellipse. wheel_y<0: scroll down, wheel_y>0: scroll up, wheel_x<0:
                      // scroll right, wheel_x>0: scroll left.
...
struct ImGuiInputTextCallbackData
{
  ...
  // 'Buf[]' directly. You need to update_ellipse 'BufTextLen' accordingly (0 <= BufTextLen < BufSize) and set
...
// '#define ImDrawCallback MyDrawCallback' (in imconfig.h) + update_ellipse rendering backend accordingly.
...
enum ImDrawFlags_
{
  ...
  ImDrawFlags_RoundCornersBottomRight = 1
      << 7, // AddRect(), AddRectFilled(), PathRect(): enable rounding bottom-right corner only (when rounding > 0.0f,
...
enum ImDrawListFlags_
{
  ...
  ImDrawListFlags_AntiAliasedLinesUseTex = 1
      << 1, // Enable anti-aliased lines/borders using textures when possible. Require backend to render with bilinear
...
struct ImFontConfig
{
  ...
  float RasterizerDensity;  // 1.0f     // DPI scale for rasterization, not altering other font metrics: make it easy to
                            // swap between e.g. a 100% and a 400% fonts for a zooming display. IMPORTANT: If you
...
namespace ImGui
{
  ...
  IMGUI_API bool ImageButton(ImTextureID user_texture_id,
                             ...
                             int frame_padding      = -1,
                             const ImVec4& bg_col   = ImVec4(0, 0, 0, 0),
                             const ImVec4& tint_col = ImVec4(1, 1, 1, 1)); // Use new ImageButton() signature (explicit
  ...
  // height_in_items = -1) { float height = GetTextLineHeightWithSpacing() * ((height_in_items < 0 ? ImMin(items_count,
  // 7) : height_in_items) + 0.25f) + GetStyle().FramePadding.y * 2.0f; return BeginListBox(label, ImVec2(0.0f,
  // height)); } // Helper to calculate size from items_count and height_in_items static inline void  ListBoxFooter() {
  // EndListBox(); }
