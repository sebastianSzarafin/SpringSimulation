...
void ImGui_ImplSDLRenderer2_RenderDrawData(ImDrawData* draw_data)
{
  ...
  // If the user has specified a scale factor to SDL_Renderer already via SDL_RenderSetScale(), SDL will scale whatever
  // we pass to SDL_RenderGeometryRaw() by that scale factor. In that case we don't want to be also scaling it ourselves
  // here.
  float rsx = 1.0f;
  float rsy = 1.0f;
...
bool ImGui_ImplSDLRenderer2_CreateFontsTexture()
{
  ...
  io.Fonts->GetTexDataAsRGBA32(
      ...
      &height); // Load as RGBA 32-bit (75% of the memory is wasted, but default font is so small) because it is more
                ...
                // concept than just a GL texture id, consider calling GetTexDataAsAlpha8() instead to save on GPU
                // memory.
