                        ...
                        const ImVec4& tint_col)
{
  ...
  if (frame_padding >= 0) PushStyleVar(ImGuiStyleVar_FramePadding, ImVec2((float)frame_padding, (float)frame_padding));
...
bool ImGui::SliderFloatWithSteps(const char* label,
                                 float* v,
                                 float v_min,
                                 float v_max,
                                 float v_step,
                        ...
                        ImGuiInputTextFlags flags)
{
  ...
  flags |= ImGuiInputTextFlags_AutoSelectAll |
      (ImGuiInputTextFlags)ImGuiInputTextFlags_NoMarkEdited; // We call MarkItemEdited() ourselves by comparing the
...
// at the end.
// FIXME: Ideally we should transition toward (1) making InsertChars()/DeleteChars() update_ellipse undo-stack (2)
// discourage (and keep reconcile) or obsolete (and remove reconcile) accessing buffer directly.
                        ...
                        void* callback_user_data)
{
  ...
  if (g.LastItemData.InFlags & ImGuiItemFlags_ReadOnly) flags |= ImGuiInputTextFlags_ReadOnly;
  ...
  if (g.ActiveId == id)
  {
    ...
    if (apply_edit_back_to_user_buffer)
    {
            ...
            ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
      {
        ...
        else if (flags & ImGuiInputTextFlags_CallbackAlways) { event_flag = ImGuiInputTextFlags_CallbackAlways; }
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
  ...
  else if (flags & ImGuiColorEditFlags_InputHSV) { ColorConvertHSVtoRGB(H, S, V, R, G, B); }
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
  ...
  if (!display_frame &&
      (flags &
       (ImGuiTreeNodeFlags_SpanAvailWidth | ImGuiTreeNodeFlags_SpanFullWidth | ImGuiTreeNodeFlags_SpanAllColumns)) == 0)
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
  ...
  const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_NoHoldingActiveID |
      ImGuiSelectableFlags_NoSetKeyOwner | ImGuiSelectableFlags_SelectOnClick | ImGuiSelectableFlags_DontClosePopups;
...
void ImGui::EndTabBar()
{
  ...
  else { window->DC.CursorPos.y = tab_bar->BarRect.Max.y + tab_bar->PrevTabsContentsHeight; }
  ...
  if (select_dir != 0)
    if (ImGuiTabItem* tab_item = TabBarFindTabByID(tab_bar, tab_bar->SelectedTabId))
    {
      ...
      while (tab_to_scroll_to == NULL)
      {
        ...
        // (even if first/last item is a button, return it so we can update_ellipse the scroll)
                      ...
                      ImGuiWindow* docked_window)
{
  ...
  if (hovered_unblocked && (IsMouseClicked(1) || IsMouseReleased(1)) && !is_tab_button) TabBarQueueFocus(tab_bar, tab);
